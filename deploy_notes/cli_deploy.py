...
def to_cloud_run(
  ...
  * agents/[app_name]/
  ...
  try:
    ...
    region_options = f'--region {region}' if region else [] #DEAN
    project = _resolve_project(project)
    subprocess.run(
        f'gcloud run deploy {service_name} --source {temp_folder} --project {project} {region_options} --port {str(port)} --verbosity {log_level.lower() if log_level else verbosity} --labels created-by=adk'
        ,
        shell=True,
    )
  finally:
    click.echo(f'gcloud run deploy {service_name} --source {temp_folder} --project {project} {region_options} --port {str(port)} --verbosity {log_level.lower() if log_level else verbosity} --labels created-by=adk')
