...
def generate_audio(elevenlabs_client, voiceover: str, voiceid: str, verbose: bool = False) -> Dict[str, Any]:
    ...
    logging.info(f"🤖 Sending text length: {len(voiceover)} characters to ElevenLabs API...")
...
def elevenlabs_generation(script_obj: Dict[str, Any] = None, voice_name: str = None):
    """Main execution of the audio generation and segmentation pipeline."""

    elevenlabs_client = get_elevenlabs_client()
    if elevenlabs_client is None:
        return {"status": "error", "message": "ElevenLabs client initialization failed."}
    ...
    if GENERATE_NEW:
        ...
        logging.info(f"🤖 Generating audio with ElevenLabs voice: {voice_name}...")
    ...
    logging.info("🤖 Calculating scene durations from alignment data...")
    ...
    if alignment:
        ...
        for i, s in enumerate(script_obj["scenes"]):
            ...
            logging.info(f"✅ Scene {i+1} duration: {s.get('duration')}")
        print("-" * 40)
    else:
