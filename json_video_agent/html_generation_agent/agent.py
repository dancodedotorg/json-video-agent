...
3. **Co-Create Together**
    - No specific tool; you will generate HTML directly in conversation with the user
    - How to use:
      - Ask the user if they want to co-create the HTML together
      - If confirmed, use the `scenes_with_duration` object in state and generate one HTML slide per scene
        - Generate minimalist, clean HTML with CSS either in the head or inline. No javascript or external resources.
      - Preserve all original scene metadata (commentary, speech, voice settings, and duration)
      - Add a new `html` field containing the generated HTML for that scene
      - When finished, use the `commit_html_voiceover_scenes` tool to record the new html scenes to the state
    - Once you have generated the final JSON object, ask the user if they want to save the final output.
...
class HTMLScene(BaseModel):
    ...
    html: str = Field(description="A slide visual generated with HTML, displayed during the voiceover")

# 2. Define the overall structure
class HTMLSceneList(BaseModel):
    """A list of one or more scenes."""
    model_config = ConfigDict(extra="allow")
    scenes: List[HTMLScene]


def commit_html_voiceover_scenes(json_str: str, tool_context: ToolContext) -> Dict[str, Any]:
    """Updates the session state with the in-progress scenes being generated between the agent and the user.
    
    This tool commits the in-progress scenes being generated by the agent to the session state under 'html_voiceover_scenes'.
    **Input Format:** The json_str input must conform to the HTMLSceneList schema, containing an array of scenes with at least these properties:
    {
      "scenes": [
        {
          "comment": "A brief comment about the scene",
          "speech": "The voiceover speech for this scene.",
          "html": "A slide visual generated with HTML, displayed during the voiceover"
        },
        ...
      ]
    }
    ...
    # Validate the JSON against the HTMLSceneList schema
    try:
        scenes = HTMLSceneList.model_validate_json(candidate_json)
    except ValidationError as e:
        return {
            "status": "error",
            "message": f"Provided JSON does not conform to schema: {e}"
        }

    # If we made it here: it worked!
    json_obj = scenes.model_dump()
    tool_context.state["html_voiceover_scenes"] = json_obj
    return {
        "status": "success",
        "message": "Initial voiceover scenes committed to state.",
        "html_voiceover_scenes": json_obj
    }

# Helper function to generate final export object within this agent because of image gen
async def generate_final_export_obj(scenes_parent_obj, tool_context: ToolContext) -> Dict[str, Any]:
...
try:
    html_generation_agent = Agent(
        ...
        tools=[list_saved_artifacts, list_current_state, commit_html_voiceover_scenes, generate_html_from_slide_pngs, generate_html_with_image_generation, generate_final_export_obj]
    )   
